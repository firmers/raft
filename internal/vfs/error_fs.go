...
func (fs *ErrorFS) OpenReadWrite(name string, opts ...vfs.OpenOption) (vfs.File, error) {
	if err := fs.inj.MaybeError(OpWrite); err != nil {
		return nil, err
	}
...
func (fs *ErrorFS) OpenForAppend(name string) (vfs.File, error) {
	if err := fs.inj.MaybeError(OpWrite); err != nil {
		return nil, err
	}
...
func (fs *ErrorFS) GetDiskUsage(path string) (vfs.DiskUsage, error) {
	if err := fs.inj.MaybeError(OpRead); err != nil {
